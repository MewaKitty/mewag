...
//const skills = ["Arch (strength)","Bite (strength)","Climb (strength)","Focus (spirit)","Hiss (spirit)","Jump (spirit)","Listen (spirit)","Ponder (intelligence)","Pounce (intelligence)","See (intelligence)","Smell (intelligence)","Sneak (spirit)","Swat (intelligence)","Swim (spirit)","Wrestle (spirit)"]

const skills = ["Arch", "Bite", "Climb", "Focus", "Hiss", "Jump", "Listen", "Ponder", "Pounce", "See", "Smell", "Sneak", "Swat", "Swim", "Wrestle"]
...
const knacks = ["Alertness (1 SPT chip = +1 to Listen, See, Scent, Focus, or Ponder)", "Animal Lore (1 INT chip = +1 to Intelligence or Ponder involving animals)", "Balance (1 SPT chip = +1 to Climb or Strength)", "Belly Rake (2 STR chip = Belly Rake attack, Swat but +1 damage)", "Chomp (3 STR chip = Chomp attack, Pins enemy)", "Clan Lore (1 INT chip = +1 to Intelligence or Ponder)", "Dash (1 STR chip = Move quickly)", "Dodge (1 SPT chip = +1 to Jump)", "Feint (1 INT chip = +1 to Swat, improves chances but doesn't add damage)", "Herb Lore (1 INT chip = +1 to Intelligence or Ponder involving herbs)", "Hide (1 SPT chip = +2 to Sneak)", "Interpret Dreams (2 SPT chip = Understand dreams)", "Launch (3 STR chip = Launch attack, needs you and enemy be wrestling)", "Leap (1 STR chip = Pounce further)", "Medicine Lore (1 INT chip = +1 to Intelligence or Ponder involving knows what a is ailing a sick cat)", "Mighty Swat (1 STR chip = +1 attack damage to Swat)", "Orate (1 SPT chip = +1 to Spirit, Focus, or Ponder)", "Pathfinder (1 INT chip = Know distance to known place and known places near you)", "Pin (3 STR chip = Pin attack, make enemy not able to move)", "Pummel (3 STR chip = Make Swat use a strength check instead of intelligence)", "Spring (3 STR chip = Go to a high object)", "Stalk (1 SPT chip = +2 to Sneak)", "Track (1 INT chip = +1 to Listen, See or Smell)", "Twoleg Lore (1 INT chip = +1 to Intelligence or Ponder involving Twolegs", "Yowl (1 INT = +1 to Arch or Hiss)"]
...
["strength", "intelligence", "spirit"].forEach((c, i) => {
  document.getElementById(c + "Plus2").addEventListener("click", () => {
    if (abilityPoints > 0) {
      abilityPoints--
      cat.abilities[i]++
      document.getElementById("abilityUpgrades").innerHTML = abilityPoints
...
for (let i = 0; i < parsedSkills.length; i += 5) {
  ...
  chunk.forEach(c => {
    ...
    skillPart.innerHTML = `<b>${c}</b><span class="skillType">(${["Strength","Intelligence","Spirit"][skillData[skills.indexOf(c)]]})</span><span id="${c.toLowerCase()}Count" class="skillCount"></span><button id="${c.toLowerCase()}Plus" class="plus">+</button><button id="${c.toLowerCase()}Minus" class="minus">-</button>`
    column++;
    if (skillRows[column] === undefined) { skillRows[column] = document.createElement("tr"); }
...
skills.forEach((c, i) => {
  ...
  document.getElementById(c + "Plus").addEventListener("click", () => {
    if (skillPoints > 0) {
      ...
      document.getElementById(c + "Count").innerHTML = "+" + cat.skills[i]
    }
  })
  document.getElementById(c + "Minus").addEventListener("click", () => {
    if (cat.skills[i] > 0 && (cat.skills[i] > 1 ? true : { "ThunderClan": 7, "RiverClan": 13, "WindClan": 6, "ShadowClan": 11 } [cat.clan] !== i)) {
      skillPoints++
      cat.skills[i]--
      skillPointsC.innerHTML = skillPoints
      document.getElementById(c + "Count").innerHTML = cat.skills[i] > 0 ? "+" + cat.skills[i] : ""
    }
  })
})
...
knacks.forEach((c, i) => {
  ...
  knackDiv.addEventListener("click", () => {
    let knackIndex = cat.knacks[0] !== undefined ? (cat.knacks[1] !== undefined ? 2 : 1) : 0
...
knacks.forEach((c, i) => {
  ...
  knackDiv.addEventListener("click", () => {
    ...
    } else if (knackPoints > 0 && (knackData[i].upgrade === -1 ? Infinity : knackData[i].upgrade) > cat.knackLevels[i]) {
      cat.knackLevels[i]++
      document.getElementById("knacku" + cat.knacks[i]).innerHTML = c + " [Level " + cat.knackLevels[i] + "]"
      knackPoints--
    }
  })
...
for (let i = 0; i < parsedUpgradeSkills.length; i += 5) {
  ...
  chunk.forEach(c => {
    const skillPart = document.createElement("td")
    skillPart.innerHTML = `<b>${c}</b><span class="skillType">(${["Strength","Intelligence","Spirit"][skillData[skills.indexOf(c)]]})</span><span id="${c.toLowerCase()}Count2" class="skillCount"></span><button id="${c.toLowerCase()}Plus2" class="plus">+</button><button id="${c.toLowerCase()}Minus2" class="minus">-</button>`
    column++;
    if (upgradeSkillRows[column] === undefined) { upgradeSkillRows[column] = document.createElement("tr"); }
...
skills.forEach((c, i) => {
  ...
  document.getElementById(c + "Minus2").addEventListener("click", () => {
    if (cat.skills[i] > 0 && (cat.skills[i] > 1 ? true : { "ThunderClan": 7, "RiverClan": 13, "WindClan": 6, "ShadowClan": 11 } [cat.clan] !== i)) {
      ...
      cat.skills[i]--
      skillPointsU.innerHTML = skillPoints
      document.getElementById(c + "Count2").innerHTML = cat.skills[i] > 0 ? cat.skills[i] : ""
    }
  })
})

let skillUpgraderCallback = () => {}
document.getElementById("doneUpgradingSkills").addEventListener("click", () => skillUpgraderCallback())
const buttons = document.getElementById("buttons")

const skillUpgrader = document.getElementById("skillUpgrader")

let knackUpgraderCallback = () => {}
document.getElementById("doneUpgradingKnacks").addEventListener("click", () => knackUpgraderCallback())

const knackUpgrader = document.getElementById("knackUpgrader")

let abilityUpgraderCallback = () => {}
document.getElementById("doneUpgradingAbilities").addEventListener("click", () => abilityUpgraderCallback())

const abilityUpgrader = document.getElementById("abilityUpgrader")

let intsptUpgraderCallback = () => {}
document.getElementById("doneUpgradingIntspt").addEventListener("click", () => intsptUpgraderCallback())
...
const abilityCheck = async (c) => {
  for (const x of Object.entries(c.outcomes).sort((a, b) => b[0] - a[0])) {
    if (!Array.isArray(c.value)) {
      if (cat.abilities[c.value] + (c.change ? varScope[c.change] : 0) >= x[0]) {
        await run(x[1])
        break;
      }
    } else {
...
const skillCheckBtns = async (c, varScope, fromCheck, fromScript) => {
  if (Array.isArray(c.value)) {
    ...
    for (const x of Object.entries(c.outcomes).sort((a, b) => Number(b[0]) - Number(a[0]))) {
      if (sum - (c.change && !fromCheck ? varScope[c.change] : 0) >= x[0]) {
        await run(x[1])
        break;
      }
    }
  ...
  resultElem.innerHTML = "Result of Check: " + (cat.skills[c.value] + cat.abilities[skillData[c.value]])
  ...
  createButton(skills[c.value], async () => {
    if (fromCheck) {
      ...
      const check = cat.skills[c.value] + cat.abilities[skillData[c.value]] + bonus - (c.change && !fromCheck ? varScope[c.change] : 0)
      for (const x of Object.entries(c.outcomes).sort((a, b) => Number(b[0]) - Number(a[0]))) {
        if (check >= x[0]) {
          await run(x[1])
          break;
  ...
  createButton("Use 1 " + ["Strength", "Intelligence", "Spirit"][skillData[c.value]] + " Chip", () => {
    switch (skillData[c.value]) {
      case 0:
        if (strChips > 0) {
          strChips--
          currentStrength.style.width = `${(strChips/cat.abilities[0]) * 100}%`
      ...
      case 1:
        if (intChips > 0) {
          intChips--
          currentIntelligence.style.width = `${(intChips/cat.abilities[1]) * 100}%`
      ...
      case 2:
        if (sptChips > 0) {
          sptChips--
          currentSpirit.style.width = `${(sptChips/cat.abilities[2]) * 100}%`
          currentSpirit.innerHTML = `${sptChips}/${cat.abilities[2]} SPT Chips`
          bonus++
        }
    }
    resultElem.innerHTML = "Result of Check: " + (cat.skills[c.value] + cat.abilities[skillData[c.value]] + bonus - (c.change && !fromCheck ? varScope[c.change] : 0))
  })
  for (let x of Object.values(cat.knacks)) {
    if ((knackData[x].category === c.category || knackData[x].category === "all") && knackData[x].spentOn.includes(c.value)) {
      createButton(`${knackData[x].name} (${knackData[x].cost - (knackData[x].upgradeReward === "cost" ? cat.knackLevels[x] : 0)} ${["STR","INT","SPT"][knackData[x].costType]} Chip = +${knackData[x].upgradeReward === "bonusIncrease" ? cat.knackLevels[x] : 1})`, () => {
        switch (knackData[x].costType) {
          case 0:
            if (strChips > knackData[x].cost - (knackData[x].upgradeReward === "cost" ? cat.knackLevels[x] : 0)) {
              strChips -= knackData[x].cost - (knackData[x].upgradeReward === "cost" ? cat.knackLevels[x] : 0)
              currentStrength.style.width = `${(strChips/cat.abilities[0]) * 100}%`
              currentStrength.innerHTML = `${strChips}/${cat.abilities[0]} STR Chips`
              bonus += knackData[x].upgradeReward === "bonusIncrease" ? cat.knackLevels[x] : 1
            }
            break
          case 1:
            if (intChips > knackData[x].cost - (knackData[x].upgradeReward === "cost" ? cat.knackLevels[x] : 0)) {
              intChips -= knackData[x].cost - (knackData[x].upgradeReward === "cost" ? cat.knackLevels[x] : 0)
              currentIntelligence.style.width = `${(intChips/cat.abilities[1]) * 100}%`
              currentIntelligence.innerHTML = `${intChips}/${cat.abilities[1]} INT Chips`
              bonus += knackData[x].upgradeReward === "bonusIncrease" ? cat.knackLevels[x] : 1
            }
            break
          case 2:
            if (sptChips > knackData[x].cost - (knackData[x].upgradeReward === "cost" ? cat.knackLevels[x] : 0)) {
              sptChips -= knackData[x].cost - (knackData[x].upgradeReward === "cost" ? cat.knackLevels[x] : 0)
              currentSpirit.style.width = `${(sptChips/cat.abilities[2]) * 100}%`
              currentSpirit.innerHTML = `${sptChips}/${cat.abilities[2]} SPT Chips`
              bonus += knackData[x].upgradeReward === "bonusIncrease" ? cat.knackLevels[x] : 1
            }
...
const abilityCheckBtns = async (c, varScope, fromCheck, fromScript) => {
  ...
  clearButtons()
  const resultElem = document.createElement("span")
  resultElem.innerHTML = "Result of Check: " + (cat.abilities[c.value] - (c.change && !fromCheck ? varScope[c.change] : 0))
  buttons.appendChild(resultElem)
  const breakElem = document.createElement("div")
  breakElem.classList.add("break")
  buttons.appendChild(breakElem)
  ...
  createButton(["Strength", "Intelligence", "Spirit"][c.value], async () => {
    if (fromCheck) {
      ...
      const check = cat.abilities[c.value] + bonus + (c.change && !fromCheck ? varScope[c.change] : 0)
      for (const x of Object.entries(c.outcomes).sort((a, b) => Number(b[0]) - Number(a[0]))) {
        if (check >= x[0]) {
          await run(x[1])
        }
  ...
  createButton("Use 1 " + ["Strength", "Intelligence", "Spirit"][c.value] + " Chip", () => {
    switch (c.value) {
      case 0:
        if (strChips > 0) {
          strChips--
    ...
    resultElem.innerHTML = "Result of Check: " + (cat.abilities[c.value] + bonus - (c.change && !fromCheck ? varScope[c.change] : 0))
  })
  for (let x of Object.values(cat.knacks)) {
    if (knackData[x].spentAbil !== undefined) {
      if ((knackData[x].category === c.category || knackData[x].category === "all") && knackData[x].spentAbil.includes(c.value)) {
        createButton(`${knackData[x].name} (${knackData[x].cost - (knackData[x].upgradeReward === "cost" ? cat.knackLevels[x] : 0)} ${["STR","INT","SPT"][knackData[x].costType]} Chip = +${knackData[x].upgradeReward === "bonusIncrease" ? cat.knackLevels[x] : 1})`, () => {
          switch (knackData[x].costType) {
            case 0:
              if (strChips > knackData[x].cost - (knackData[x].upgradeReward === "cost" ? cat.knackLevels[x] : 0)) {
  ...
  if (fromCheck) {
    ...
    return cat.abilities[c.value] + bonus - (c.change && !fromCheck ? varScope[c.change] : 0)
  }
}
  ...
  for (let c of actions) {
    ...
    if (c.type === "message") {
      ...
      for (let x of c.value) {
        ...
        } else if (x.type ? x.type === "ability" : c.method === "ability") {
          createButton(["Strength", "Intelligence", "Spirit"][x.value], async () => {
      ...
      await run(c.outcomes[Math.floor(Math.random() * c.outcomes.length)])
      ...
      while (1) {
        if (!c.goesFirst || c.round !== 0) {
          ...
          if (c.canbite || c.canbite === "undefined"  || c.canbite === "null") {
            createButton("Bite", () => {
              anAnswer = "Bite"
              res1()
            })
          ...
          if (Object.values(cat.knacks).includes(15)) {
            createButton(cat.knackLevels[15] === 2 ? "Mighty Swat (1 STR chip = 2 damage)" : "Mighty Swat (2 STR Chip = 2 damage)", async () => {
          ...
          if (c.round >= (c.rounds === null || c.rounds === undefined ? Infinity : c.rounds)) {
          ...
          if (anAnswer === "Swat" || anAnswer === "Bite") {
            if (cat.skills[12] + (pummel ? cat.abilities[1] : cat.abilities[skillData[12]]) >= c.jump) {
              let strengthCheck = cat.abilities[0]

              if (cat.skills[12] + (pummel ? cat.abilities[1] : cat.abilities[skillData[12]]) >= c.jump * 2) {
                strengthCheck += 2
              }

              if (strengthCheck <= (c.defenseStrength ? c.defenseStrength : c.strength)) {
                c.health -= (anAnswer === "Bite" ? 2 : 1 + mightySwatBonus)
              } else if (strengthCheck <= (c.defenseStrength ? c.defenseStrength : c.strength) * 2) {
                c.health -= (anAnswer === "Bite" ? 3 : 2 + mightySwatBonus)
              } else if (strengthCheck > (c.defenseStrength ? c.defenseStrength : c.strength) * 2) {
                c.health -= (anAnswer === "Bite" ? 4 : 3 + mightySwatBonus)
              }
            } else if (c.onattackfailturns) {
              for (let i = 0; i < c.onattackfailturns; i++) {
                if (c.attack === "swat") {
                  if (c.swat >= (c.firstattacknoescape && c.round === 1 ? 0 : cat.skills[12] + cat.abilities[skillData[12]])) {
                    let strengthCheck = c.strength

                    if (c.swat >= cat.skills[12] + cat.abilities[skillData[12]] * 2) {
                      strengthCheck += 2
                    }
                  ...
                  if (c.bite >= (c.firstattacknoescape && c.round === 1 ? 0 : cat.skills[12] + cat.abilities[skillData[12]])) {
                    let strengthCheck = c.strength

                    if (c.bite >= cat.skills[12] + cat.abilities[skillData[12]] * 2) {
                      strengthCheck += 2
                    }
            ...
            if (cat.skills[5] + cat.abilities[skillData[5] >= 3]) {
              if (c.onescape !== null && c.onescape !== undefined) {
              ...
              if (c.onfailescape !== null && c.onfailescape !== undefined) {
...
You have ${sptChips} spirit chips.`)
            ...
            if (anAnswer2 === "Strength") {
              if (strChips >= 1) {
                ...
                setGameText("You don't have enough strength chips.")
                clearButtons()
                let res1 = () => {}
                const promise = new Promise((res, rej) => res1 = res)
                createButton("Ok", res1)
              ...
              if (intChips >= 1) {
                ...
                setGameText("You don't have enough intelligence chips.")
                clearButtons()
                let res1 = () => {}
                const promise = new Promise((res, rej) => res1 = res)
                createButton("Ok", res1)
...
You have ${sptChips} spirit chips.`)
      ...
      let healType = healTypes[Math.floor(Math.random() * healTypes.length)]
